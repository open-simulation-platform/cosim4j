...
object CosimLibrary {
    ...
    private val LOG: Logger = LoggerFactory.getLogger(CosimLibrary::class.java)

    external fun getLastError(): CosimError
     ...
     * @return A pointer to an object which holds the execution state, or NULL on error.
     */
    private external fun createFixedStepSspExecution(
        sspDir: String,
        stepSize: Double,
        startTimeDefined: Boolean,
        startTime: Double
    ): ExecutionPtr
     ...
     * @return A pointer to an object which holds the execution state, or NULL on error.
     */
    private external fun createOspConfigExecution(
        sspDir: String,
        startTimeDefined: Boolean,
        startTime: Double
    ): ExecutionPtr
    ...
    /**
     * Starts an execution.
     * The execution will run until `cosim_execution_stop()` is called.
     *
     ...
     */
    external fun getStatus(execution: ExecutionPtr): CosimExecutionStatus?
     ...
     * @return true on success and false on error.
     */
    external fun getSlaveInfos(execution: ExecutionPtr): List<CosimSlaveInfo>
     ...
     *
     */
    external fun getIntegerSamples(
        observer: ObserverPtr,
        slaveIndex: Int,
        vr: Long,
        stepNumber: Long,
     ...
     * @return true on success and false on error.
     */
    external fun getStepNumbersForDuration(
        observer: ObserverPtr,
        slaveIndex: Int,
        duration: Double,
        steps: LongArray
    ...
    fun startObserving(observer: ObserverPtr, slaveIndex: Int, type: CosimVariableType, vr: Long): Boolean {
     ...
     */
    external fun stopObserving(observer: ObserverPtr, slaveIndex: Int, type: CosimVariableType, vr: Long): Boolean
    ...
    external fun getModelDescription(execution: ExecutionPtr, slaveIndex: Int): CosimModelDescription
     ...
     */
    fun setLogLevel(level: CosimLogLevel) {
    ...
    init {
        ...
        fun loadLib(libName: String) {
            ...
            CosimLibrary::class.java.classLoader.getResourceAsStream(relativeLibPath)?.use {
        ...
        try {
            ...
            setLogLevel(CosimLogLevel.COSIM_LOG_SEVERITY_INFO)
