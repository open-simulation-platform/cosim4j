...
cosim::variable_causality get_causality(JNIEnv* env, jclass Fmi2ScalarVariable, jobject scalarVariable)
{
    ...
    switch (result) {
        case 0: return cosim::variable_causality::parameter;
        case 1: return cosim::variable_causality::calculated_parameter;
        case 2: return cosim::variable_causality::input;
        case 3: return cosim::variable_causality::output;
        default: return cosim::variable_causality::local;
    }
}

cosim::variable_variability get_variability(JNIEnv* env, jclass Fmi2ScalarVariable, jobject scalarVariable)
{
    ...
    switch (result) {
        case 0: return cosim::variable_variability::constant;
        case 1: return cosim::variable_variability::fixed;
        case 2: return cosim::variable_variability::tunable;
        case 3: return cosim::variable_variability::discrete;
        default: return cosim::variable_variability::continuous;
    }
}

cosim::variable_type get_type(JNIEnv* env, jclass Fmi2ScalarVariable, jobject scalarVariable)
{
    ...
    if (real != nullptr) return cosim::variable_type ::real;
    ...
    if (integer != nullptr) return cosim::variable_type ::integer;
    ...
    if (boolean != nullptr) return cosim::variable_type ::boolean;
    ...
    if (string != nullptr) return cosim::variable_type ::string;

    throw std::invalid_argument("Unknown variable_type encountered!");
}

cosim::variable_description to_variable_description(JNIEnv* env, jobject var)
{
    ...
    vd.reference = static_cast<cosim::value_reference>(env->CallLongMethod(var, referenceId_));
...
jvm_slave::jvm_slave(JNIEnv* env, jobject slave)
{
    ...
    jmethodID defineId = GetMethodID(env, cls, "__define__", "()Lno/ntnu/ihb/fmi4j/export/fmi2/Fmi2Slave;");
    env->CallObjectMethod(slave_, defineId);

    setupExperimentId_ = GetMethodID(env, cls, "setupExperiment", "(D)V");
    enterInitialisationModeId_ = GetMethodID(env, cls, "enterInitialisationMode", "()V");
...
cosim::model_description jvm_slave::model_description() const
{
    return model_description_;
}

void jvm_slave::setup(cosim::time_point startTime, std::optional<time_point> /*stopTime*/, std::optional<double> /*relativeTolerance*/)
{
...
cosim::step_result jvm_slave::do_step(cosim::time_point currentT, cosim::duration deltaT)
{
    bool status;
    double currentTimeSec = to_double_time_point(currentT);
    double stepSizeSec = to_double_duration(deltaT, {});
    jvm_invoke(jvm_, [this, &status, currentTimeSec, stepSizeSec](JNIEnv* env) {
        status = env->CallBooleanMethod(slave_, doStepId_, currentTimeSec, stepSizeSec);
    });
    return status ? cosim::step_result::complete : cosim::step_result::failed;
}
